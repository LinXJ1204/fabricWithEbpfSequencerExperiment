...
do
  ...
  run_cmd_on_device 0 "cd mainPlan/fabricWithEbpfSequencerExperiment/test-network/experiments/ex1 && export GO111MODULE=on && go mod tidy && nohup go run . $((250*i)) > eBPF_latency_${i}.log 2>&1 &"
  ...
  run_cmd_on_device 5 "cd mainPlan/fabricWithEbpfSequencerExperiment/test-network/experiments/TPSmeasure && nohup go run . > eBPF_TPS_${i}.log 2>&1 &"

  # 4) Wait 4 minutes
  echo -e "\n>>> Waiting 4 minutes..."
  sleep 300  # 240 seconds = 4 minutes
