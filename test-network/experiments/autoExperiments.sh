...
do
  ...
  run_cmd_on_device 0 "cd mainPlan/fabricWithEbpfSequencerExperiment/test-network/experiments/ex1 && export GO111MODULE=on && go mod tidy && nohup go run . $((250*i)) > eBPF_512_run_latency_${i}.log 2>&1 &"
  ...
  run_cmd_on_device 1 "cd mainPlan/fabricWithEbpfSequencerExperiment/test-network/experiments/TPSmeasure && nohup /usr/local/go/bin/go run . > eBPF_512_run_tps_${i}.log 2>&1 &"
