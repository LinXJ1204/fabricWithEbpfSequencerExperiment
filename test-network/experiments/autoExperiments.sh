...
do
  ...
  run_cmd_on_device 0 "cd mainPlan/fabricWithEbpfSequencerExperiment/test-network/experiments/ex1 && export GO111MODULE=on && go mod tidy && nohup go run . $((2**i)) > ex1_run_tps_${i}.log 2>&1 < /dev/null &"
  ...
  run_cmd_on_device 2 "cd mainPlan/fabricWithEbpfSequencerExperiment/test-network/experiments/TPSmeasure && nohup go run . > ex1_run_latency_${i}.log 2>&1 < /dev/null &"
