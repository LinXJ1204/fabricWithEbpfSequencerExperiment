...
func newGrpcConnection(_gatewayPeer string, _peerEndpoint string) *grpc.ClientConn {
	...
	transportCredentials := credentials.NewClientTLSFromCert(certPool, _gatewayPeer)

	connection, err := grpc.Dial(_peerEndpoint, grpc.WithTransportCredentials(transportCredentials))
	if err != nil {
