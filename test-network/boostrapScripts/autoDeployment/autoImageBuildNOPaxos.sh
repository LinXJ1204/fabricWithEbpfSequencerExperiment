...
run_cmd_on_device 0 "cd mainPlan/fabricWithEbpfSequencer && git checkout feat/nopaxos && make docker"

# 2) bringUpNode/orderer1.sh on Device1
run_cmd_on_device 1 "cd mainPlan/fabricWithEbpfSequencer && git checkout feat/nopaxos && make docker"

# 3) bringUpNode/orderer2.sh on Device2
run_cmd_on_device 2 "cd mainPlan/fabricWithEbpfSequencer && git checkout feat/nopaxos && make docker"

# 4) sudo bringUpNode/orderer3.sh on Device3
run_cmd_on_device 3 "cd mainPlan/fabricWithEbpfSequencer && git checkout feat/nopaxos && make docker"

# 5) sudo bringUpNode/orderer4.sh on Device4
run_cmd_on_device 4 "cd mainPlan/fabricWithEbpfSequencer && git checkout feat/nopaxos && make docker"
