...
osnadmin channel join --channelID ${channel_name} --config-block ${PWD}/../../channel-artifacts/${channel_name}.block -o localhost:10053 --ca-file "$ORDERER_CA" --client-cert "$ORDERER_ADMIN_TLS_SIGN_CERT" --client-key "$ORDERER_ADMIN_TLS_PRIVATE_KEY"